...
$device_build_number = strtok(substr($UserAgent, strpos($UserAgent, ";") + 1), ")")."<br>";
$deviceType = 9;
if (str_contains($device_build_number, "Win32") or strpos($device_build_number, "Win64")) {
$deviceType = 2; //Windows
}elseif (str_contains($device_build_number, "HarmonyOS")){
  $deviceType = 4; //HarmonyOS
}elseif (str_contains($device_build_number, "Android")) {
  $deviceType = 0; //Android
}elseif (str_contains($device_build_number, "iPhone")) {
  $deviceType = 1; // iPhone
}elseif (str_contains($device_build_number, "Mac OS X")) {
  $deviceType = 3; // MacOS
}elseif (str_contains($device_build_number, "Linux")) {
  $deviceType = 5; // MacOS
}

require_once 'funcs/gen_token.php';
if(!isset($_COOKIE['userID'])) {
  $tempTokenp1 = getToken(20);
  $tempTokenp2 = strrev(substr($tempTokenp1,0,10));
  $userID = "2n8".$tempTokenp1.$tempTokenp2."Cq6";
